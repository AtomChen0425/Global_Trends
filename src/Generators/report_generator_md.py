...
from FetchPipeline.fetch_from_web import fetch_url_content
...
def generate_report(intel: dict) -> str:
    """生成 Markdown 报告"""
...
def save_report(content: str, output_dir: str = "reports") -> str:
    ...
    date_str = datetime.now().strftime("%Y-%m-%d")
    file_path = os.path.join(output_dir, f"Daily_Briefing_{date_str}.md")
